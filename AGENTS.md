...
- **Package id is `bolt12-pay`.** Dependent of `lnd`: it reaches LND's REST and gRPC over the LXC bridge (importing `controlHostId` / `gRPCHostId` / `gRPCInterfaceId` / `lndconnectRestId` from `lnd-startos/startos/interfaces`, resolved in `startos/utils.ts` via `sdk.host.get`) and injects those addresses into `assets/start.sh` as `LND_REST_URL` / `LND_GRPC_ADDRESS` — the app and the embedded `lndk` read them from there (the `lnd.startos` DNS name is gone in StartOS 0.4.0). Reads LND's TLS cert + admin macaroon off a read-only dependency mount at `/mnt/lnd`.
